...
const CONTEXT_URL =
  "https://schema.opencerts.io/certificate-of-awards/2.0/context.json";

const ajv = new Ajv({ strictSchema: false, allowUnionTypes: true, strictTuples: false, allErrors: true });
addFormats(ajv);
const validator = ajv.compile(schema);

// A minimal W3C VC certificate-of-award used as the base for mutation tests.
const initialData = {
  "@context": [
...
describe("certificate-of-awards/v2.0 (W3C VC)", () => {
  ...
  describe("schema.json validation", () => {
    ...
    describe("credentialSubject", () => {
      ...
      // context.json defines every term inside the type-scoped "CertificateOfAward"
      ...
      it("should fail when the credentialSubject type does not include CertificateOfAward", () => {
      ...
      it("should be valid when the credentialSubject type is a bare string", () => {
        const data = set(cloneDeep(initialData), "credentialSubject.type", "CertificateOfAward");
        expect(validator(data)).toBe(true);
      });
      ...
      it("should fail when signature name is missing", () => {
        const data = omit(cloneDeep(initialData), "credentialSubject.signature.name");
        expect(validator(data)).toBe(false);
      });

      it("should fail when award has neither achievementYear nor achievementDate", () => {
        const data = omit(cloneDeep(initialData), [
          "credentialSubject.award.achievementDate",
          "credentialSubject.award.achievementYear",
        ]);
        expect(validator(data)).toBe(false);
      });
      ...
      it("should fail when achievementYear is not a 4 digit year", () => {
        const data = omit(cloneDeep(initialData), "credentialSubject.award.achievementDate");
        set(data, "credentialSubject.award.achievementYear", "20222");
        expect(validator(data)).toBe(false);
      });
    });
  ...
  describe("selective disclosure (deriveCredential)", () => {
    it("reveals only the selected fields, hides the rest, and still verifies", async () => {
      ...
      expect(derived.derived.credentialSubject.award).toBeUndefined();
  ...
  describe("schema.json accepts nothing that cannot be signed", () => {
    const MUTATIONS = [
      ...
      ["award with no achievement date", (r) => {
        unset(r, "credentialSubject.award.achievementDate");
        unset(r, "credentialSubject.award.achievementYear");
      }],
      ["unknown key in award", (r) => set(r, "credentialSubject.award.zzz", "v")],
      ["signature without a name", (r) => unset(r, "credentialSubject.signature.name")],
      ["unknown key in signature", (r) => set(r, "credentialSubject.signature.zzz", "v")],
    ];
